    ...
    value: function parseSelect(select) {
      ...
      if (_lodash2['default'].isPlainObject(select)) {
        return _lodash2['default'].map(select, function (value, key) {
          return _this4.parser.parseSelect(value) + (' AS "' + key + '"');
        });
      } else if (_lodash2['default'].isArray(select)) {
        return select.map(function (value) {
          return _this4.parser.parseSelect(value) + (' AS "' + value + '"');
        });
      ...
      if (_lodash2['default'].isPlainObject(obj)) {
        return _lodash2['default'].mapValues(includes, function (fieldInc, fieldName) {
          if (_lodash2['default'].isPlainObject(fieldInc)) {
            return _this5.refineR(obj[fieldName], fieldInc);
          } else {
        ...
        return obj.map(function (v) {
          return _this5.refineR(v, includes);
        });
      }
    }
  }]);
...
var Scope = (function () {
  ...
  _createClass(Scope, [{
    ...
    value: function readIncludes(baseIncludes) {
      ...
      var includes = this.orm.includesResolver.mergeVirtualFieldsDependencies(this.model, baseIncludes);
      ...
      (0, _lodash2['default'])(this.model.relations).pick(function (relation, fieldName) {
        return includes[fieldName];
      }).forEach(function (relation, fieldName) {
        if (!relation.isCollection) {
          if (relation.foreignKey && _lodash2['default'].isEqual(includes[fieldName], { id: true })) {
            _this6.fetchedFields[fieldName] = {
              alias: _this6.queryHandler.nextAlias(),
            ...
            _this6.resolveScope(fieldName, true, includes[fieldName]);
          }
        } else {
          _this6.subsequentFetches[fieldName] = includes[fieldName];
        }
      }).value();
    ...
    value: function mergeIncludes(target, source) {
      ...
      _lodash2['default'].forEach(source, function (value, key) {
        if (!target[key] || target[key] === true) {
          ...
          target[key] = _this7.mergeIncludes(target[key], source[key]);
        }
      });
      return target;
    }
    ...
    value: function resolveSelect() {
      ...
      return [].concat(_toConsumableArray(_lodash2['default'].map(this.fetchedFields, function (field) {
        return _this8.alias + '.' + (0, _clientJs.escapeId)(field.column) + ' AS ' + field.alias;
    ...
    value: function resolveSubsequentFetches(rows) {
      ...
      var promises = [].concat(_toConsumableArray(_lodash2['default'].map(this.subsequentFetches, function (cfg, fieldName) {
        return _this9.model._resolver(fieldName).resolve(rows, cfg);
    ...
    value: function resolveVirtualFields(row) {
      ...
      this.virtualFields.forEach(function (fieldName) {
        row[fieldName] = _this10.model.virtualFields[fieldName].getter.call(row);
      });
  ...
  _createClass(Parser, [{
    ...
    value: function parseOrder(input) {
      ...
      return Normalizer.order(input).map(function (ordering) {
        return _this11.expression(ordering.expr) + ' ' + (ordering.asc ? 'ASC' : 'DESC');
      }).join(', ');
    }
  }, {
    key: 'parseGroup',
    value: function parseGroup(input) {
      ...
      return Normalizer.group(input).map(function (expr) {
        return _this12.expression(expr);
      }).join(', ');
    }
  ...
  predicates: function predicates(_predicates, context) {
    ...
    if (_lodash2['default'].isArray(_predicates)) {
      return _predicates.map(function (predicate) {
        return _this15.predicate(predicate, context);
      ...
      return _lodash2['default'].map(_predicates, function (value, key) {
        return _this15.predicateWithKey(value, key, context);
      }).filter(_lodash2['default'].identity);
    } else {
