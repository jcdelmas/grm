...
var Scope = (function () {
  ...
  _createClass(Scope, [{
    key: 'readIncludes',
    value: function readIncludes(baseIncludes) {
      var _this4 = this;

      var includes = _lodash2['default'].isPlainObject(baseIncludes) ? (0, _lodash2['default'])(this.model.virtualFields).filter(function (cfg, field) {
        return baseIncludes[field];
      }).map('dependsOn').reduce(this.mergeIncludes, _lodash2['default'].cloneDeep(baseIncludes)) : {};

      if (!includes.hasOwnProperty('$defaults')) {
        includes.$defaults = true;
      }

      this.fetchedFields = (0, _lodash2['default'])(this.model.fields).pick(function (cfg, fieldName) {
        return includes.$defaults || includes[fieldName];
      }).mapValues(function (fieldCfg) {
        var fieldAlias = _this4.queryHandler.nextAlias();
        return {
          alias: fieldAlias,
          transform: fieldCfg.getter || _lodash2['default'].identity,
          column: fieldCfg.column
        };
      }).value();

      _lodash2['default'].forEach(this.model.relations, function (relation, fieldName) {
        if (includes[fieldName]) {
          if (!relation.isCollection) {
            _this4.resolveScope(fieldName, true, includes[fieldName]);
          } else {
            _this4.subsequentFetches[fieldName] = includes[fieldName];
          }
      ...
      this.virtualFields = (0, _lodash2['default'])(this.model.virtualFields).pick(function (cfg, fieldName) {
        return cfg.include && includes.$defaults || includes[fieldName];
      }).keys().value();
    }
  }, {
    key: 'mergeIncludes',
    ...
    value: function resolveSelect() {
      ...
      return [].concat(_toConsumableArray(_lodash2['default'].map(this.fetchedFields, function (field) {
        return _this6.alias + '.' + (0, _clientJs.escapeId)(field.column) + ' AS ' + field.alias;
