  ...
  _createClass(QueryHandler, [{
    ...
    value: function execute() {
      ...
      return this.orm.client.query(query).then(function (rawResults) {
        ...
        if (_this.hasModelResult()) {
          return _this.rootScope.resolveSubsequentFetches(rows).then(function () {
            rows.forEach(function (row) {
              return _this.rootScope.resolveVirtualFields(row);
            });
            return rows;
          });
        } else {
    ...
    value: function rowParser() {
      ...
      if (this.query.count || this.query.select) {
        return this.hasScalarResult() ? function (row) {
          return row.value;
    ...
    value: function parseSelect(select) {
      ...
      if (_lodash2['default'].isPlainObject(select)) {
        return _lodash2['default'].map(select, function (value, key) {
          return _this3.parser.parseSelect(value) + (' AS "' + key + '"');
        });
      } else if (_lodash2['default'].isArray(select)) {
        return select.map(function (value) {
          return _this3.parser.parseSelect(value) + (' AS "' + value + '"');
        });
      } else if (_lodash2['default'].isString(select)) {
    ...
    key: 'hasScalarResult',
    value: function hasScalarResult() {
      return this.query.count || this.query.select && _lodash2['default'].isString(this.query.select);
    }
  }, {
    key: 'hasModelResult',
...
var Scope = (function () {
  ...
  function Scope(queryHandler, model, includes) {
    ...
    this.fetchedFields = {};
    this.virtualFields = (0, _lodash2['default'])(this.model.virtualFields).pick(function (cfg) {
      return cfg.include;
    }).keys().value();
  ...
  _createClass(Scope, [{
    key: 'readIncludes',
    value: function readIncludes(includes) {
      var _this4 = this;

      if (_lodash2['default'].isPlainObject(includes)) {
        var fullIncludes = (0, _lodash2['default'])(this.model.virtualFields).filter(function (cfg, field) {
          return includes[field];
        }).map('dependsOn').reduce(this.mergeIncludes, _lodash2['default'].cloneDeep(includes));

        _lodash2['default'].forEach(fullIncludes, function (fieldIncludes, fieldName) {
          if (fieldIncludes) {
            if (_this4.model.relations[fieldName]) {
              var relation = _this4.model.relations[fieldName];
              if (!relation.isCollection) {
                _this4.resolveScope(fieldName, true, fieldIncludes);
              } else {
                _this4.subsequentFetches[fieldName] = fieldIncludes;
              }
            } else if (_this4.model.virtualFields[fieldName]) {
              _this4.virtualFields.push(fieldName);
            } else {
              throw new Error('Unknown field [' + fieldName + '] for model [' + _this4.model.name + ']');
            }
          }
        });
      }
    ...
    key: 'mergeIncludes',
    value: function mergeIncludes(target, source) {
      var _this5 = this;

      _lodash2['default'].forEach(source, function (value, key) {
        if (!target[key] || target[key] === true) {
          target[key] = source[key];
        } else if (_lodash2['default'].isObject(source[key])) {
          target[key] = _this5.mergeIncludes(target[key], source[key]);
        }
      });
      return target;
    }
    ...
    value: function resolveSelect() {
      ...
      return [].concat(_toConsumableArray(_lodash2['default'].map(this.model.fields, function (fieldCfg, fieldName) {
        var fieldAlias = _this6.queryHandler.nextAlias();
        _this6.fetchedFields[fieldName] = {
          alias: fieldAlias,
          transform: _this6.model.fields[fieldName].getter || _lodash2['default'].identity
        };
        return _this6.alias + '.' + (0, _clientJs.escapeId)(fieldCfg.column) + ' AS ' + fieldAlias;
      })), _toConsumableArray((0, _lodash2['default'])(this.model.relations).pick(function (relation, fieldName) {
        return (!_this6.children[fieldName] || !_this6.children[fieldName].fetched) && relation.foreignKey;
      }).map(function (relation, fieldName) {
        var fieldAlias = _this6.queryHandler.nextAlias();
        _this6.fetchedFields[fieldName] = {
          alias: fieldAlias,
        ...
        return _this6.alias + '.' + (0, _clientJs.escapeId)(relation.foreignKey) + ' AS ' + fieldAlias;
      }).value()), _toConsumableArray((0, _lodash2['default'])(this.getFetchedChildren()).map(function (child) {
    ...
    value: function resolveScope(fieldName) {
      var fetched = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];
      var includes = arguments.length <= 2 || arguments[2] === undefined ? true : arguments[2];
      ...
      if (!this.children[fieldName]) {
        if (this.model.relations[fieldName]) {
          ...
          var scope = new Scope(this.queryHandler, relationModel, includes);
    ...
    value: function parseRow(row) {
      var result = _lodash2['default'].mapValues(this.fetchedFields, function (_ref) {
        var alias = _ref.alias;
        var transform = _ref.transform;

        return transform(row[alias]);
      });
      var relations = _lodash2['default'].mapValues(this.getFetchedChildren(), function (child) {
    ...
    value: function resolveSubsequentFetches(rows) {
      ...
      var promises = [].concat(_toConsumableArray(_lodash2['default'].map(this.subsequentFetches, function (cfg, fieldName) {
        return _this7.model._resolver(fieldName).resolve(rows, cfg);
    ...
    value: function resolveVirtualFields(row) {
      ...
      this.virtualFields.forEach(function (fieldName) {
        row[fieldName] = _this8.model.virtualFields[fieldName].getter.call(row);
      });
    ...
    value: function getFetchedChildren() {
      return (0, _lodash2['default'])(this.children).pick(function (c) {
        return c.fetched;
  ...
  _createClass(Parser, [{
    ...
    value: function parseOrder(input) {
      ...
      return Normalizer.order(input).map(function (ordering) {
        return _this9.expression(ordering.expr) + ' ' + (ordering.asc ? 'ASC' : 'DESC');
      }).join(', ');
    }
  }, {
    key: 'parsegroup',
    value: function parsegroup(input) {
      ...
      return Normalizer.group(input).map(function (expr) {
        return _this10.expression(expr);
      }).join(', ');
    }
