...
class QueryHandler {
  ...
  execute() {
    ...
    const groupByPart = this.query.groupBy ? this.parser.parseGroupBy(this.query.groupBy) : null;
...
class Parser {
  ...
  parseGroupBy(input) {
    return Normalizer.groupBy(input).map(expr => this.expression(expr)).join(', ');
  }
  ...
  expression(expr) {
    if (expr instanceof Field) {
      ...
      return expr.fn + '(' + distinct + expr.args.map(arg => this.expression(arg)).join(', ') + ')';
